        ...
        <form method="POST" action="{{ route('perfil.update') }}" class="rounded-xl bg-white p-6 shadow-sm">
            ...
            <div class="grid gap-4 sm:grid-cols-2">
                <label class="block sm:col-span-2">
                    ...
                    <input name="name" value="{{ old('name', $user->name) }}" required maxlength="255" class="w-full rounded-lg border-gray-300">
                </label>
                <label class="block">
                    <span class="mb-1 block text-sm font-semibold text-gray-700">Correo</span>
                    <input value="{{ $user->email }}" disabled class="w-full rounded-lg border-gray-200 bg-gray-50 text-gray-500">
                </label>
                <label class="block">
                    <span class="mb-1 block text-sm font-semibold text-gray-700">Número de contacto</span>
                    <input name="numero_contacto" value="{{ old('numero_contacto', $user->numero_contacto) }}" maxlength="20" class="w-full rounded-lg border-gray-300">
                </label>
            </div>
            ...
            <div class="grid gap-4">
                ...
                <div class="grid gap-4 sm:grid-cols-2">
                    ...
                    <label class="block">
                        <span class="mb-1 block text-sm font-semibold text-gray-700">Confirmar nueva contraseña</span>
                        <input type="password" name="new_password_confirmation" class="w-full rounded-lg border-gray-300">
                    </label>
                </div>
            </div>

            <button class="mt-6 rounded-full bg-[#8A486F] px-6 py-3 font-bold text-white hover:bg-[#733b5c]">Guardar cambios</button>
        </form>

        <aside class="h-fit rounded-xl bg-white p-6 shadow-sm">
            <div class="mb-5 flex h-20 w-20 items-center justify-center rounded-full bg-[#8A486F] text-2xl font-bold text-white">
                {{ Str::of($user->name)->substr(0, 1)->upper() }}
            </div>
            <h2 class="text-xl font-bold">{{ $user->name }}</h2>
            <p class="text-sm text-gray-500">{{ $user->email }}</p>

            <form method="POST" action="{{ route('perfil.destroy') }}" class="mt-8" onsubmit="return confirm('¿Eliminar tu cuenta definitivamente?')">
                @csrf
                @method('DELETE')
                <label class="block">
                ...
                <button class="mt-3 w-full rounded-full border border-red-300 px-4 py-2 font-semibold text-red-700 hover:bg-red-50">Eliminar cuenta</button>
            </form>
        </aside>
    </div>
</main>
