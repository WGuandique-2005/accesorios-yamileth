...
<main class="min-h-screen p-4 md:ml-64 md:p-8">
    <div class="mb-8 flex flex-col gap-3 lg:flex-row lg:items-end lg:justify-between">
        <div>
            ...
            <p class="mt-2 text-gray-600">Ganancias, inversión y comportamiento de ventas con la misma estética de la tienda.</p>
        </div>
    ...
    <section class="grid gap-4 sm:grid-cols-2 xl:grid-cols-4">
        ...
        <div class="rounded-xl bg-white p-5 shadow-sm">
            ...
            <p class="mt-2 text-3xl font-bold text-[#8A486F]">${{ number_format($currentSummary['revenue'], 2) }}</p>
        </div>
        <div class="rounded-xl bg-white p-5 shadow-sm">
            ...
            <div class="flex h-72 items-end gap-3 border-b border-gray-200">
                @foreach ($weeklyProfitChart as $item)
                    ...
                    <div class="flex flex-1 flex-col items-center justify-end gap-2">
                        <span class="text-xs font-semibold {{ $isPositive ? 'text-green-700' : 'text-red-700' }}">${{ number_format($item['value'], 0) }}</span>
                        <div class="w-full rounded-t-lg {{ $isPositive ? 'bg-[#8A486F]' : 'bg-red-400' }}" style="height: {{ $height }}%"></div>
        ...
        <div class="rounded-xl bg-white p-6 shadow-sm">
            ...
            @if ($topClient)
                <div class="mt-4 rounded-lg bg-[#FDF0F4] p-5">
                    ...
                    <p class="mt-2 text-gray-600">${{ number_format($topClient->total_comprado, 2) }} en {{ $topClient->pedidos }} pedidos</p>
                </div>
            @else
