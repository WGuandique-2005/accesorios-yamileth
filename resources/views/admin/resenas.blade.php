...
<main class="min-h-screen p-4 md:ml-64 md:p-8">
    <div class="mb-8 flex flex-col gap-3 lg:flex-row lg:items-end lg:justify-between">
        <div>
            ...
            <p class="mt-2 text-gray-600">Aquí ves los comentarios y calificaciones que dejan los clientes después de recibir su pedido.</p>
        </div>
        <a href="{{ route('admin.dashboard') }}" class="w-fit rounded-full border border-[#8A486F] px-5 py-3 font-bold text-[#8A486F]">Volver al dashboard</a>
    </div>

    <form method="GET" action="{{ route('admin.resenas.index') }}" class="mb-6 grid gap-4 rounded-xl bg-white p-5 shadow-sm md:grid-cols-3">
        <label class="block">
        ...
        <label class="block">
            ...
            <select name="product_id" class="w-full rounded-lg border-gray-300">
                ...
                @foreach ($products as $product)
                    <option value="{{ $product->id }}" @selected(request('product_id') == $product->id)>{{ $product->nombre }}</option>
                @endforeach
            </select>
        </label>
        <div class="flex items-end gap-3">
            <button class="rounded-full bg-[#8A486F] px-5 py-3 font-bold text-white">Aplicar</button>
            <a href="{{ route('admin.resenas.index') }}" class="rounded-full border border-gray-300 px-5 py-3 font-semibold text-gray-700">Limpiar</a>
        </div>
    </form>
                ...
                <div class="flex flex-col gap-3 lg:flex-row lg:items-start lg:justify-between">
                    <div>
                        <div class="flex flex-wrap items-center gap-3">
                            ...
                            <span class="rounded-full bg-[#FDF0F4] px-3 py-1 text-sm font-semibold text-[#8A486F]">{{ $review->rating }}/5</span>
                        </div>
                        <p class="mt-1 text-sm text-gray-600">Cliente: {{ $review->user->name }} · Pedido #{{ $review->orderItem->order->id }} · {{ $review->created_at->format('d/m/Y H:i') }}</p>
                    </div>
                    <div class="flex items-center gap-1">
                        @for ($star = 1; $star <= 5; $star++)
                            <span class="{{ $star <= $review->rating ? 'text-[#8A486F]' : 'text-gray-300' }} text-lg">★</span>
                        @endfor
                    </div>
                </div>
