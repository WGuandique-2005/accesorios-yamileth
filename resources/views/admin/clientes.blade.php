                        ...
                        <tbody class="divide-y divide-gray-100">
                            @forelse ($clients as $client)
                                <tr>
                                    ...
                                    <td class="p-4">
                                        <p>{{ $client->email }}</p>
                                        <p class="text-xs text-gray-500">{{ $client->numero_contacto ?: 'Sin teléfono' }}
                                        </p>
                                    </td>
                                    <td class="p-4">{{ $client->created_at->format('d/m/Y') }}</td>
                                    <td class="p-4">{{ $client->orders_count }}</td>
                                    <td class="p-4 font-bold text-[#8A486F]">
                                        ${{ number_format($client->total_comprado ?? 0, 2) }}</td>
                                    <td class="p-4 text-right">
                                        <a href="{{ route('admin.clientes.show', $client) }}"
                                            class="font-semibold text-[#8A486F] hover:underline">Historial</a>
                                        @if ($client->orders_count === 0)
                                            <form method="POST" action="{{ route('admin.clientes.destroy', $client) }}"
                                                class="inline" onsubmit="return confirm('¿Eliminar este cliente?')">
                                                @csrf
                                                @method('DELETE')
                                                <button
                                                    class="ml-3 font-semibold text-red-600 hover:underline">Eliminar</button>
                                            </form>
                                        @endif
                                    </td>
                                </tr>
                            @empty
                                <tr>
                                    <td colspan="6" class="p-8 text-center text-gray-500">No hay clientes registrados.</td>
                                </tr>
                            @endforelse
                        </tbody>
                    </table>
                            ...
                            <a href="{{ route('admin.pedidos.show', $order) }}" class="block py-3">
                                <div class="flex justify-between">
                                    <div>
                                        ...
                                        <p class="text-xs text-gray-500">{{ $order->fecha->format('d/m/Y') }} -
                                            {{ $order->estado }}</p>
                                    </div>
