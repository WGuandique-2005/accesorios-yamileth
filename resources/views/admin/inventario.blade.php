...
<main class="min-h-screen p-4 md:ml-64 md:p-8">
    <div class="mb-8 flex flex-col gap-4 lg:flex-row lg:items-center lg:justify-between">
        ...
        <a href="#product-form" class="w-fit rounded-full bg-[#8A486F] px-5 py-3 font-bold text-white">Nuevo producto</a>
    </div>
    ...
    <section id="product-form" class="mb-8 rounded-xl bg-white p-6 shadow-sm">
        ...
        <form method="POST" action="{{ $product ? route('admin.inventario.update', $product) : route('admin.inventario.store') }}" enctype="multipart/form-data" class="grid gap-4 lg:grid-cols-4">
            @csrf
            @if ($product) @method('PUT') @endif
            <label class="block lg:col-span-2">
                <span class="mb-1 block text-sm font-semibold text-gray-700">Nombre</span>
                <input name="nombre" value="{{ old('nombre', $product?->nombre) }}" required maxlength="150" class="w-full rounded-lg border-gray-300">
            </label>
            <label class="block">
                <span class="mb-1 block text-sm font-semibold text-gray-700">Stock</span>
                <input type="number" name="cantidad_stock" value="{{ old('cantidad_stock', $product?->cantidad_stock ?? 0) }}" min="0" required class="w-full rounded-lg border-gray-300">
            </label>
            <label class="flex items-end gap-2 pb-3">
                <input type="checkbox" name="activo" value="1" @checked(old('activo', $product?->activo ?? true)) class="rounded border-gray-300 text-[#8A486F]">
            ...
            <label class="block">
                ...
                <input type="number" step="0.01" name="precio_inversion" value="{{ old('precio_inversion', $product?->precio_inversion ?? 0) }}" min="0" required class="w-full rounded-lg border-gray-300">
            </label>
            <label class="block">
                <span class="mb-1 block text-sm font-semibold text-gray-700">Precio venta</span>
                <input type="number" step="0.01" name="precio_unitario" value="{{ old('precio_unitario', $product?->precio_unitario ?? 0) }}" min="0" required class="w-full rounded-lg border-gray-300">
            </label>
            <label class="block">
            ...
            <div class="lg:col-span-4 flex gap-3">
                ...
                @if ($product)
                    <a href="{{ route('admin.inventario.index') }}" class="rounded-full border border-gray-300 px-6 py-3 font-semibold text-gray-700">Cancelar</a>
                @endif
            </div>
        </form>
    </section>
                ...
                <tbody class="divide-y divide-gray-100">
                    @forelse ($products as $item)
                        <tr class="{{ $item->trashed() ? 'bg-gray-50 text-gray-400' : '' }}">
                            ...
                            <td class="p-4">
                                @if (! $item->trashed())
                                    <button data-toggle-url="{{ route('admin.inventario.toggle', $item) }}" class="toggle-btn rounded-full px-3 py-1 text-xs font-bold {{ $item->activo ? 'bg-green-100 text-green-700' : 'bg-gray-100 text-gray-600' }}">{{ $item->activo ? 'Activo' : 'Oculto' }}</button>
                                @else
                            ...
                            <td class="p-4 text-right">
                                @if (! $item->trashed())
                                    <a href="{{ route('admin.inventario.edit', $item) }}" class="font-semibold text-[#8A486F] hover:underline">Editar</a>
                                    <form method="POST" action="{{ route('admin.inventario.destroy', $item) }}" class="inline" onsubmit="return confirm('¿Eliminar este producto?')">
                                        @csrf
                                        @method('DELETE')
