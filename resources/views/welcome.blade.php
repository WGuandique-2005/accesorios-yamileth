...
<body class="bg-background text-on-background font-body-md min-h-screen flex flex-col antialiased">
    ...
    <main class="flex-grow">
        ...
        <section class="hero-pattern relative overflow-hidden pt-20 pb-32 px-margin-mobile md:px-margin-desktop">
            <div
                class="max-w-container-max mx-auto text-center flex flex-col items-center justify-center relative z-10">
                <span
                    class="font-label-caps text-label-caps text-primary tracking-[0.2em] mb-6 uppercase block">Colección
                    boutique</span>
                ...
                <p class="font-body-lg text-body-lg text-on-surface-variant max-w-xl mx-auto mb-10">
                    Descubre piezas lindas diseñadas para resaltar. Variedad de accesorios que no encontraras en otro
                    lugar.
                </p>
                <div class="flex flex-col sm:flex-row gap-4 justify-center items-center">
                    <a href="{{ route('login') }}"
                        class="bg-primary text-on-primary px-8 py-4 rounded-full font-body-md font-medium hover:shadow-lg hover:-translate-y-1 transition-all duration-300 w-full sm:w-auto flex items-center gap-2 justify-center">
                        Explorar Colección
                        <span class="material-symbols-outlined text-sm"
                            style="font-variation-settings: 'wght' 600;">arrow_forward</span>
                    </a>
                    <a href="{{ route('login') }}"
                        class="px-8 py-4 rounded-full font-body-md font-medium text-primary border border-primary/20 hover:bg-primary/5 transition-colors w-full sm:w-auto">
                        Ver Novedades
                    </a>
                </div>
            </div>
            <!-- Decorative Elements -->
            <div
                class="hero-orb absolute top-1/2 left-10 w-64 h-64 bg-primary-container/20 rounded-full -translate-y-1/2">
            </div>
            <div
                class="hero-orb absolute top-1/2 right-10 w-72 h-72 bg-tertiary-container/20 rounded-full -translate-y-1/2">
            </div>
        </section>
            ...
            <div class="max-w-container-max mx-auto">
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-gutter">
                    @forelse ($products as $product)
                        <article
                            class="ghost-card bg-surface rounded-xl overflow-hidden group flex flex-col relative h-[450px]">
                            @if(\Carbon\Carbon::parse($product->created_at)->diffInDays(now()) < 30)
                                <span
                                    class="absolute top-4 left-4 z-10 bg-tertiary text-on-tertiary px-3 py-1 rounded-full font-label-caps text-label-caps">NUEVO</span>
                            @endif
                            <button
                                class="absolute top-4 right-4 z-10 w-10 h-10 bg-surface/80 backdrop-blur-sm rounded-full flex items-center justify-center text-on-surface-variant hover:text-primary transition-colors">
                                <span class="material-symbols-outlined text-xl">favorite</span>
                            </button>
                            <div class="h-2/3 w-full bg-surface-variant relative overflow-hidden">
                                <img alt="{{ $product->nombre }}"
                                    class="w-full h-full object-cover group-hover:scale-105 transition-transform duration-700 ease-out"
                                    src="{{ asset('storage/' . $product->imagen_ruta) }}" />
                            </div>
                            <div class="p-6 flex-grow flex flex-col justify-between text-center">
                                <div>
                                    ...
                                    <p class="font-body-md text-body-md text-primary font-medium">
                                        ${{ number_format($product->precio_final, 2) }}</p>
                                </div>
                                <button
                                    class="text-primary font-medium font-body-sm tracking-wide uppercase hover:text-primary-fixed-variant transition-colors flex items-center justify-center gap-2 mt-4 group/btn">
                                    Hacer encargo
                                    <span
                                        class="material-symbols-outlined text-sm group-hover/btn:translate-x-1 transition-transform">trending_flat</span>
                                </button>
                            </div>
                        </article>
                    @empty
                        <div class="col-span-1 md:col-span-2 lg:col-span-3 text-center py-12">
                            <p class="font-h3 text-h3 text-on-surface-variant">Pronto habrá productos disponibles para ti 🌸
                            </p>
                        </div>
                    @endforelse
                </div>
                <div class="mt-16 text-center">
                    <a href="{{ route('login') }}"
                        class="px-8 py-3 rounded-full font-body-md font-medium text-primary border border-primary hover:bg-primary hover:text-on-primary transition-all duration-300 inline-flex items-center justify-center">
        ...
        <section class="py-20 bg-surface border-t border-outline-variant/30">
            <div
                class="max-w-container-max mx-auto px-margin-mobile md:px-margin-desktop grid grid-cols-1 md:grid-cols-3 gap-12 text-center">
                <div class="flex flex-col items-center">
                    <div
                        class="w-16 h-16 bg-primary-container/30 rounded-full flex items-center justify-center text-primary mb-4">
                        <span class="material-symbols-outlined text-3xl"
                            style="font-variation-settings: 'wght' 300;">diamond</span>
                    </div>
                    <h4 class="font-h3 text-h3 mb-2 text-on-surface">Calidad Premium</h4>
                    <p class="font-body-sm text-body-sm text-on-surface-variant">Materiales seleccionados cuidadosamente
                        para asegurar durabilidad y belleza.</p>
                </div>
                <div class="flex flex-col items-center">
                    <div
                        class="w-16 h-16 bg-primary-container/30 rounded-full flex items-center justify-center text-primary mb-4">
                        <span class="material-symbols-outlined text-3xl"
                            style="font-variation-settings: 'wght' 300;">local_shipping</span>
                    </div>
                    <h4 class="font-h3 text-h3 mb-2 text-on-surface">Envíos Seguros</h4>
                    <p class="font-body-sm text-body-sm text-on-surface-variant">Entregas confiables para que tus piezas
                        lleguen en perfectas condiciones.</p>
                </div>
                <div class="flex flex-col items-center">
                    <div
                        class="w-16 h-16 bg-primary-container/30 rounded-full flex items-center justify-center text-primary mb-4">
                        <span class="material-symbols-outlined text-3xl"
                            style="font-variation-settings: 'wght' 300;">redeem</span>
                    </div>
                    <h4 class="font-h3 text-h3 mb-2 text-on-surface">Empaque Especial</h4>
                    <p class="font-body-sm text-body-sm text-on-surface-variant">Cada pedido se prepara como cariño,
                        ideal para sorprender o consentirte.</p>
                </div>
            </div>
        </section>
    </main>
