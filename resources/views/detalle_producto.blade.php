...
<body class="min-h-screen bg-[#FFF8F8] text-[#201A1D]" style="font-family: Inter, sans-serif;">
    ...
    <main class="mx-auto max-w-6xl px-4 py-10 sm:px-6 lg:px-8">
        ...
        <a href="{{ route('home') }}"
            class="mb-8 inline-flex text-sm font-semibold text-[#8A486F] hover:underline">Volver al catálogo</a>
        ...
        <section class="grid gap-10 lg:grid-cols-2 lg:items-start">
            <div class="rounded-xl bg-white p-4 shadow-sm">
                <div class="relative overflow-hidden rounded-xl bg-[#FDF0F4]">
                    <img id="main-product-image"
                        src="{{ $gallery->first() ? asset('storage/' . $gallery->first()) : asset('images/logo.jpeg') }}"
                        alt="{{ $product->nombre }}" class="aspect-square w-full object-cover">
                    @if ($gallery->count() > 1)
                        <button type="button" data-carousel-prev
                            class="absolute left-3 top-1/2 -translate-y-1/2 rounded-full bg-white/90 p-2 shadow-md text-[#8A486F]">‹</button>
                        <button type="button" data-carousel-next
                            class="absolute right-3 top-1/2 -translate-y-1/2 rounded-full bg-white/90 p-2 shadow-md text-[#8A486F]">›</button>
                    @endif
                </div>
                @if ($gallery->count() > 1)
                    <div class="mt-4 grid grid-cols-4 gap-3">
                        @foreach ($gallery as $index => $image)
                            <button type="button" data-carousel-thumb="{{ $index }}"
                                class="overflow-hidden rounded-lg border-2 {{ $index === 0 ? 'border-[#8A486F]' : 'border-transparent' }}">
                                <img src="{{ asset('storage/' . $image) }}"
                                    alt="{{ $product->nombre }} miniatura {{ $index + 1 }}" class="h-20 w-full object-cover">
                            </button>
                        @endforeach
                    </div>
                @endif
            ...
            <div class="rounded-xl bg-white p-6 shadow-sm sm:p-8">
                ...
                <div class="mt-6 flex flex-wrap items-end gap-3">
                    <span
                        class="text-4xl font-bold text-[#8A486F]">${{ number_format($product->precio_final, 2) }}</span>
                    @if ($product->descuento > 0)
                        <span class="rounded-full bg-green-100 px-3 py-1 text-sm font-semibold text-green-700">Descuento de
                ...
                <div class="mt-4 flex flex-col gap-1 text-sm text-gray-600">
                    <div class="flex items-center gap-1"
                        aria-label="Calificación {{ number_format($product->promedio_calificacion ?: 0, 1) }} de 5">
                        @for ($star = 1; $star <= 5; $star++)
                            <span
                                class="{{ $star <= round($product->promedio_calificacion ?: 0) ? 'text-[#8A486F]' : 'text-gray-300' }} text-lg">★</span>
                        @endfor
                    </div>
                    <span>{{ number_format($product->promedio_calificacion ?: 0, 1) }}/5 · {{ $product->total_resenas }}
                        reseñas</span>
                </div>
                ...
                <a href="{{ route('encargos.create', ['producto' => $product->id]) }}"
                    class="mt-8 inline-flex w-full items-center justify-center rounded-full bg-[#8A486F] px-6 py-4 text-lg font-bold text-white hover:bg-[#733b5c]">
                    Hacer encargo
                </a>
            </div>
        </section>
        ...
        <section class="mt-10 rounded-xl bg-white p-6 shadow-sm">
            <div class="flex items-center justify-between gap-4">
                ...
                <div class="text-right">
                    <p class="text-2xl font-bold text-[#8A486F]">
                        {{ number_format($product->promedio_calificacion ?: 0, 1) }}</p>
                    ...
                    <article class="rounded-lg bg-[#FDF0F4] p-4">
                        <div class="flex items-center justify-between gap-3">
                            ...
                            <div class="flex items-center gap-0.5" aria-label="Calificación {{ $review->rating }} de 5">
                                @for ($star = 1; $star <= 5; $star++)
                                    <span
                                        class="{{ $star <= $review->rating ? 'text-[#8A486F]' : 'text-gray-300' }} text-sm">★</span>
                                @endfor
                            </div>
                        </div>
