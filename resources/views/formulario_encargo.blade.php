    ...
    <main class="mx-auto max-w-6xl px-4 py-10 sm:px-6 lg:px-8">
        ...
        <form method="POST" action="{{ route('encargos.store') }}" class="grid gap-8 lg:grid-cols-[1fr_360px]">
                ...
                <section class="rounded-xl bg-white p-6 shadow-sm">
                    ...
                    <div class="grid gap-4 sm:grid-cols-[1fr_130px_auto]">
                        ...
                        <button type="button" id="addProductBtn"
                            class="rounded-lg bg-[#8A486F] px-4 py-2 font-semibold text-white hover:bg-[#733b5c]">Agregar</button>
                    </div>
                ...
                <section class="rounded-xl bg-white p-6 shadow-sm">
                    ...
                    <div class="grid gap-4 sm:grid-cols-2">
                        ...
                        <label class="block">
                            ...
                            <input name="lugar_despacho" value="{{ old('lugar_despacho') }}" maxlength="100"
                                class="w-full rounded-lg border-gray-300" placeholder="Melo Express, Metrocentro...">
                        </label>
                        <label class="block sm:col-span-2">
                            <span class="mb-1 block text-sm font-semibold text-gray-700">Lugar de recibir</span>
                            <input name="lugar_de_recibir" value="{{ old('lugar_de_recibir') }}" maxlength="255"
                        ...
                        <label class="block">
                            ...
                            <input type="date" name="fecha" value="{{ old('fecha', now()->toDateString()) }}"
                                min="{{ now()->toDateString() }}" required class="w-full rounded-lg border-gray-300">
                        </label>
                        <label class="block">
                            <span class="mb-1 block text-sm font-semibold text-gray-700">Hora recordatorio</span>
                            <input type="time" name="hora_recordatorio" value="{{ old('hora_recordatorio') }}"
                                class="w-full rounded-lg border-gray-300">
                        </label>
                        <label class="block sm:col-span-2">
                            <span class="mb-1 block text-sm font-semibold text-gray-700">Notas</span>
                            <textarea name="notas" rows="3" maxlength="500" class="w-full rounded-lg border-gray-300"
            ...
            <aside class="h-fit rounded-xl bg-white p-6 shadow-sm lg:sticky lg:top-24">
                ...
                <button
                    class="mt-6 w-full rounded-full bg-[#8A486F] px-6 py-3 font-bold text-white hover:bg-[#733b5c]">Confirmar
                    encargo</button>
            </aside>
        </form>
    </main>
